...
next.onclick = nextSlide;   // 다음 이미지 표사
...
function showSlides(n) {
    ...
    current++;
    if (current > slides.length)      // 마지막 이지미라면
        current = 1;
...
function prevSlide() {
    ...
    else current = slides.length - 1;       // 그렇지 않으면(첫 번쩨 이미지이므로) 마지막 위치로
    showSlides(current);                    // 이동한 위치의 표시
}
